...
PlayerPtr Ball::updatePosition(float elapsed_time, const PlayerPtr& player_1, const PlayerPtr& player_2)
{
  ...
  if( !playerCollide( player_1, player_1->rightLimit() + radius() ) )
    playerCollide( player_2, player_2->leftLimit() - radius() );
...
{
  if( collide(position(), enlarge(player->position(), player->size(), radius())) )
  {
